...
## 8. Варианты сборки и внешнее Android-приложение (Dual-Build Architecture)

Начиная с версии v4.0.5, система поддерживает два варианта компиляции прошивки, адаптированных под различные сценарии развертывания и ограничения памяти:
...
```
       ...
       [ENABLE_EMBEDDED_WEB_UI]                  [#undef ENABLE_EMBEDDED_WEB_UI]
  ┌─────────────────────────────────┐       ┌─────────────────────────────────┐
  │      AgriSwarm-WebUI            │       │      AgriSwarm-AppReady         │
  ├─────────────────────────────────┤       ├─────────────────────────────────┤
  │ • Встроенный веб-сайт во Flash  │       │ • Нет статических файлов        │
  │ • Standby-режим HTML (10/90)    │       │ • Активны только REST API + SSE │
  │ • Доступен прямо из браузера    │       │ • Управление через Android-App  │
  │ • Расход Flash: +80KB           │       │ • Максимальная экономия RAM     │
  └─────────────────────────────────┘       └─────────────────────────────────┘
```

### 8.1. Версия AgriSwarm-WebUI (Встроенный интерфейс)
*   **Флаг сборки:** `#define ENABLE_EMBEDDED_WEB_UI`
*   **Особенности:** Включает в себя сжатый веб-интерфейс в `PROGMEM` (`include/WebRes.h`). Узлом можно полностью управлять с любого устройства, просто подключившись к его точке доступа.
*   **Применение:** Идеально для небольших сетей (1-3 узла), где пользователю важно быстро зайти на плату с любого телефона без установки приложений.

### 8.2. Версия AgriSwarm-AppReady (Легковесная под Android-приложение)
*   **Флаг сборки:** `#undef ENABLE_EMBEDDED_WEB_UI` (или `#define CONFIG_DISABLE_EMBEDDED_WEB_UI`)
*   **Особенности:** Полностью исключает файлы фронтенда из Flash. Асинхронный веб-сервер обслуживает только REST API-эндпоинты (`/api/status`, `/api/pins`, `/api/rules`, `/api/cli`) и SSE-канал событий `/events`.
*   **Применение:** Идеально для крупных mesh-сетей (5+ узлов), где экономия каждого килобайта RAM критически важна для стабильности маршрутизации. Управление осуществляется через внешнее Android/PWA-приложение, установленное на смартфоне.

### 8.3. Взаимодействие с Android-приложением
Внешнее Android-приложение работает локально на мобильном устройстве и общается с платой по беспроводной сети:
1.  **Прямой режим (Direct Wi-Fi):** Приложение подключается к точке доступа узла `AgriSwarmMesh` по стандартному IP `10.0.0.1`.
2.  **Сетевой режим (Local Network):** Приложение работает в общей Wi-Fi сети хозяйства, отправляя запросы на IP-адрес шлюза, выданный роутером.
3.  **Формат обмена:** 
    *   **Управление:** POST-запросы с JSON-телом для переключения пинов (`/api/pins/toggle`) и добавления правил (`/api/rules/add`).
    *   **Мониторинг:** Потоковое получение логов и изменений показаний датчиков в реальном времени через канал **Server-Sent Events (SSE)** `/events`.
