...
flowchart TD
    ...
    D --> E{Подключено?}
    E -->|❌ Нет| C
    E -->|✅ Да| F[🌡️ pin setup dht22 4 temp1<br/>Настройка датчика]
    F --> G[🔌 pin setup relay 5 fan1<br/>Настройка реле]
    G --> H[🤖 rule add temp1 > 25 fan1 on<br/>Создание правила]
    H --> I[✅ sensor read temp1<br/>Проверка датчика]
...
**Типичные ошибки:**
- ❌ Забыли подключить датчик физически → sensor read показывает NaN
...
# - Проверьте датчик: sensor read [имя]
...
# отдельные команды (status, sensor read) вместо monitor.
#
...
**Как защититься:**
- ✅ Всегда проверяйте результат командой `status` или `sensor list`
...
sensor read temperature_sensor_greenhouse_north

# Начните печатать и нажмите Tab:
sen<Tab>        # → sensor
sensor re<Tab>  # → sensor read
sensor read te<Tab>  # → sensor read temperature_sensor_greenhouse_north
```
...
|-------------|----------------|----------------|
| Простые | 50KB | sensor read, relay status, pin info |
